...
<section class="mb-10 max-w-md mx-auto mt-10 p-6 bg-brand-card rounded-lg shadow-lg">
    ...
    <form wire:submit.prevent="authenticate">
        @csrf
        <div class="mb-4">
            <input wire:model.live="email" type="email" placeholder="Email" required class="w-full px-3 py-2 rounded bg-brand-dark border border-brand-muted text-brand-light focus:outline-none focus:ring-2 focus:ring-brand-accent focus:border-brand-accent">
        </div>

        <div class="mb-4">
            <input wire:model.live="password" type="password" placeholder="Senha" required class="w-full px-3 py-2 rounded bg-brand-dark border border-brand-muted text-brand-light focus:outline-none focus:ring-2 focus:ring-brand-accent focus:border-brand-accent">
            @error('password') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
        </div>

        <div class="mb-4 flex items-center gap-2">
            <input wire:model.live="remember" type="checkbox" id="remember" class="rounded border-gray-300">
            <label for="remember" class="text-brand-light text-sm">Lembrar-me</label>
        </div>

        <button type="submit" class="w-full bg-brand-accent hover:bg-brand-glow text-white font-bold py-2 px-4 rounded transition mb-3">
