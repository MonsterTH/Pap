    ...
    <main class="max-w-5xl mx-auto px-4 md:px-8 py-10">
        ...
        <div class="fade-up fade-up-d1 bg-brand-card border border-white/5 rounded-2xl p-6 shadow-lg relative overflow-visible z-10">
            ...
            <div class="fade-up fade-up-d2 flex items-center gap-4">
                <img src="{{ $profileuser->profile_picture ? (Str::startsWith($profileuser->profile_picture, 'http') ? $profileuser->profile_picture : asset('storage/' . $profileuser->profile_picture )) : asset('storage/images/default.png') }}" class="w-16 h-16 rounded-full object-cover border border-white/10">

                <div>
                    <h1 class="text-xl font-bold">{{ $profileuser->name }}</h1>
                    <p class="text-sm text-brand-muted">{{ $profileuser->created_at->format('j F, Y') }}</p>
                    <p class="text-sm text-brand-light">{{ $profileuser->dateofentry }}</p>
                </div>


                <!-- FOLLOW BUTTON -->
                <livewire:follow-button :userId="$profileuser->id" />
            </div>
            ...
            @if($post->image == Null)
                <div class="max-w-lg mx-auto bg-brand-card border border-white/5 rounded-2xl p-6 shadow-lg">
                    ...
                    <div class="flex items-center gap-3 mb-3">
                        <img src="{{ $profileuser->profile_picture ? (Str::startsWith($profileuser->profile_picture, 'http') ? $profileuser->profile_picture : asset('storage/' . $profileuser->profile_picture )) : asset('storage/images/default.png') }}"
