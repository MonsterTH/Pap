...
<main class="max-w-7xl mx-auto px-4 md:px-8 py-10 grid md:grid-cols-3 gap-8">
        ...
        <div class="bg-brand-card border border-white/5 rounded-2xl p-6 shadow-lg h-fit">
            <img class="w-16 h-16 rounded-full bg-white/10 mb-4" src="{{ auth()->user()->profile_picture ? (Str::startsWith($post->user->profile_picture, 'http') ? $post->user->profile_picture : asset('storage/' . $post->user->profile_picture )) : asset('storage/images/default.png') }}">
        ...
        <div class="md:col-span-2 space-y-6">
            ...
            <div class="flex items-center gap-3 bg-brand-card border border-white/5 rounded-xl p-4">
                ...
                <div class="flex items-center gap-3">
                    <img class="w-10 h-10 rounded-full bg-white/10" src="{{ auth()->user()->profile_picture ? (Str::startsWith($post->user->profile_picture, 'http') ? $post->user->profile_picture : asset('storage/' . $post->user->profile_picture )) : asset('storage/images/default.png') }}">
                    <div>
            ...
            <div class="bg-brand-card border border-white/5 rounded-2xl p-6 shadow-lg mb-4">
                <div class="flex items-center gap-3 mb-4">
                    <img class="w-10 h-10 rounded-full bg-white/10" src="{{ auth()->user()->profile_picture ? (Str::startsWith(auth()->user()->profile_picture, 'http') ? auth()->user()->profile_picture : asset('storage/' . auth()->user()->profile_picture)) : asset('storage/images/default.png') }}">
