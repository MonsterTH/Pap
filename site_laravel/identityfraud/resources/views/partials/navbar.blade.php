<nav class="sticky top-0 z-50 bg-brand-deeper/90 backdrop-blur-xl border-b border-white/5">
    <div class="max-w-7xl mx-auto px-4">
        <ul class="flex items-center justify-center gap-1 md:gap-2 py-3 overflow-x-auto text-sm md:text-base font-display">
                ...
                <li><a href="{{ route('login') }}" class="ml-2 px-4 py-2 bg-brand-accent/10 border border-brand-accent/30 rounded-full text-brand-accent font-semibold tracking-wide hover:bg-brand-accent/20 transition-all">Bem-vindo, {{ auth()->user()->name ?? 'Guest' }}</a></li>
        </ul>
    </div>
</nav>
